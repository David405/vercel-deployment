...
export class UserService {
	...
	async createUser(
		...
		if (!userData.message || typeof userData.message !== 'string') {
			throw CustomError.BadRequest('Invalid message', 'Missing or invalid message');
		}

		if (!userData.signature || typeof userData.signature !== 'string') {
			throw CustomError.BadRequest('Invalid signature', 'Missing or invalid signature');
		}
		// Prepare user data
		...
		const newUser = await this.userRepository.createUser(userDataForRepo, web3AccountData);
